...
- Executor
  ...
  - To run on your own HPC cluster, configure it once with
    `MitoPilot::generate_config()` and reuse it for every project. This
    saves a named profile (partition, account, container engine, etc.)
    that `new_project(executor = "<name>")` then finds automatically:
    ...
    Run `MitoPilot::list_configs()` to see all available executor names
    (built-ins plus your saved profiles). You can still pass a fully
    custom Nextflow config with `config = "path/to/.config"`.
- NCBI Api Key
