...
The current table's GEOME icons refresh when the viewer changes a sample;
other panels' tables update the next time they reload.
...
`geome_Tissue_tissueType` or `geome_Event_depthOfBottomInMeters`; the
**Template token** column shows the exact `{...}` token to paste into a header
template. Use the search box to find a field, and click rows to tick them.
...
<code>[lat_lon=]</code>, which GenBank rejects. Only add a GEOME modifier to a
...
- **Internet access is needed** by the machine running R or the app, since
  ...
  contact GEOME.
- **A failed refresh of the same BCID keeps the previous data.** If a sample
  was fetched before and a later fetch of the same BCID fails, its old
  records stay in place and remain usable at export; only the status changes
  to failed. Changing a sample to a different BCID clears its old record
  right away, even if the new fetch then fails.
...
| `BCID not recognized by GEOME` | GEOME rejected the identifier as malformed, even though it starts with `ark:/`. Check it against the record page. |
