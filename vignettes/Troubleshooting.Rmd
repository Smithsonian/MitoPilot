...
The number identifies the cause. Nextflow retries a failed process automatically
(up to a few times), and each retry requests more memory (the requested memory is
multiplied by the attempt number), so a step that fails once often succeeds on
retry. A process that fails on every retry needs your attention.
...
Because each retry requests more memory, a one-off 137 that then succeeds needs no
action. If a step fails with 137 on every retry, increase its requested memory in
the R Shiny GUI for the process that failed, then rerun the workflow (the same fix
as `exit status (140)` below).
