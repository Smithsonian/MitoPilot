...
Following the automated curation process, samples are processed through a validation script, `validate_fish_mito()` by default. This script looks for anomalies in the final annotations relative to the expectations specified in the curation parameters and add notations to assist with manual curation using the MitoPilot GUI.

+ __possible duplicate__: The annotation occurs in the assembly more often than expected. All identical annotations will receive this flag and the user must determine which (if any) should be removed.
+ __exceeds max overlap__: The annotation overlaps one or more neighboring annotations on the same strand by a percentage of its length greater than the value set by the max_overlap parameter of the curation params. This represents a global maximum overlap threshold.
...
+ __below min length__: The annotation length is below the minimum expectation set in the curation params.
...
+ __low reference similarity__: The PCG curation reference database does not include any references with compositional similarity equal to or greater than the hit_threshold set in the curation parameters (default = 90).
