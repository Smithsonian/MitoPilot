...
> tunnel](Custom-HPC.html#headless-gui-over-an-ssh-tunnel) in the Custom HPC
> vignette.
...
SEDNA does not currently support running Nextflow within the R Shiny GUI. 

For a small number of samples, you could run Nextflow in an interactive session. However, this requires you to maintain an open connection to the cluster. For large datasets, there may be issues if the connection breaks while Nextflow is running. 
...
Rather than clicking the `Run from App` button, copy the Nextflow command and create a submission script. We have provided a template below. You may wish to modify the job name (`--job-name`) and the output file names (`--output` and `--error`).
...
Move the submission script into your MitoPilot project directory (in the above example, `/home/dmacguigan/MitoPilot/test/2025_1_2/`). Then submit the job using `sbatch MY_SCRIPT_NAME.sh`.
