...
vignette: >
  %\VignetteIndexEntry{NOAA SEDNA Setup}
...
See [updating, pinning, and rolling back](Installation.html#updating-pinning-and-rolling-back).
Note that some releases change the project database schema and require
`backwards_compatibility()` before existing projects will open.

On SEDNA, activate the `MitoPilot_deps` environment first, and remember that a
stale Singularity image will keep an updated package running old pipeline code.
