...
This allows you to call `nextflow` from anywhere on the cluster. Next, let's install MitoPilot.
...
If the install was successful, you're ready to start using MitoPilot!

We recommend running MitoPilot within an interactive session on the cluster to avoid unnecessary computational burden on the login nodes. First, initialize an interactive session.

```{bash eval=F}
qrsh -l h_rt=24:00:00 
```

Note that on Hydra, interactive sessions can run for a maximum of 24 hours, and users are limited to one active interactive session at a time.
...
Follow the login instructions as presnted above. Once you have an Rstudio server window open, run `library(MitoPilot)` to load the package.
