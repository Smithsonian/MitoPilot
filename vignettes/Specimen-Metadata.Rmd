...
Click any icon in the Metadata column to open the viewer for that sample.
...
![Specimen viewer, GBIF tab, with the occurrence issues shown as badges](figures/specimen_viewer_gbif.png)
...
| not checked | MitoPilot does not judge this difference: the values are free text, or a value could not be read. Look at them yourself (shown in grey). |
...
|---|---|---|
| coordinates | Both parse (decimal, or `17.5 S 149.8 W`) and differ by at most 0.01 degree in each axis | conflict; a value that cannot be read = not checked |
| collection_date | Equal at the coarser of the two precisions (`2026` agrees with `2026-02-23`) | conflict; a value that cannot be read = not checked |
| country | Same ISO country code; names such as `USA`, `United States of America`, and `US` all map to the same code | conflict; a name MitoPilot cannot map, compared with a code, = not checked |
...
| locality, collector | Equal ignoring case and extra spaces | not checked |
| sex, dev_stage | Equal ignoring case | not checked |
...
becomes a column in the Export table, in the **Metadata** entry of the Columns
picker.
