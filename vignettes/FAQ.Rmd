...
## Can MitoPilot find the mitogenome in my whole assembly?
...
parallel, short contigs are dropped before the search, and contigs are pulled out
by name rather than by loading the file into memory. Assemblies of over a million
contigs are routine.

NUMTs are handled by requiring the match to cover a good fraction of the contig,
not just to be strong. A nuclear scaffold carrying a mitochondrial insertion
matches over a tiny slice of itself and is rejected on that basis, with the
reason recorded in the search evidence.

See ["Finding the mitogenome in a whole assembly"](Your-Own-Project.html#finding-the-mitogenome-in-a-whole-assembly)
for details.

## Can MitoPilot circularize my own assemblies?
...
Only samples marked `linear` in the mapping file are
considered. Results appear in the **Circularization** column of the Assemble
table, and the thresholds live in the **Circularize Opts.** modal. See
["Circularizing linear assemblies"](Your-Own-Project.html#circularizing-linear-assemblies)
for details.

The call is not a black box: clicking a sample's **Circularization** note opens
the overlap alignment and the junction read depth it was based on.
...
contigs. The result can be reviewed in the app and re-queued with the
**Redo Scaffold Join** button. See
["Joining fragmented assemblies"](Your-Own-Project.html#joining-fragmented-assemblies)
for details.
