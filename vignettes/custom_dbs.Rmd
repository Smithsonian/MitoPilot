...
The original shell/python workflow remains available for advanced users who want full manual control. It performs a GenBank query for all mitochondrial records matching your search criteria, downloads those sequences, and sorts them into GetOrganelle "seed" and "label" databases.
...
If you are working on the Smithsonian Hydra cluster, python and biopython are available as a module. Simply run `module load bio/biopython/1.83`.
...
This may take a while depending on how many GenBank records match your search terms. If working on a computing cluster, we recommend running this script as a batch job. Below is an example submission script for the Smithsonian Hydra cluster.
