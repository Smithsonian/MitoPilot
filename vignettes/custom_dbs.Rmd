...
It is written in pure R, so **no external tools are required** (no Entrez Direct, python, or biopython). It also validates your clade against NCBI taxonomy, checks any extra search terms before downloading, automatically decides which un-annotated sequences to keep, cleans up intermediate files, and records when GenBank was accessed.
...
custom_assembly_db(
  clade = "Asteroidea",
  db_path = "~/MitoPilot_reference_dbs", # store outside your project directories
  db_type = "both"
)
```
...
When finished, the function prints exactly how to point MitoPilot at the new databases (the `custom_seeds_db` / `custom_labels_db` / `mitofinder_db` arguments of `MitoPilot::new_project()`, or the corresponding fields in the Shiny app assembly options).

<div class="alert alert-note">
  <strong>Note on un-annotated sequences:</strong> Some mitochondrial records have no annotated genes (e.g. D-loop or poorly annotated sequences). Rather than requiring manual review, `custom_assembly_db()` automatically adds such a sequence to the seed database only if it is long enough to likely be a near-complete mitogenome (by default, at least 0.8x the median length of the complete mitogenomes in the download). Tune this with `include_nogene` and `nogene_min_frac`.
</div>
