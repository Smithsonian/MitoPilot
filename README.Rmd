...
If you do not have the raw reads for your assemblies, set `no_raw_data = TRUE`. MitoPilot will then skip read mapping and coverage calculation entirely: `data_path` is not required, your mapping file does not need `R1`/`R2` columns, and coverage/depth statistics will be left empty (annotation coverage trimming is disabled automatically). The assembly, annotation, and curation features work as usual.
