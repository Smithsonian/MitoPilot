...
test_that(".spec_compare applies each concept's rule", {
  ...
  expect_equal(cmp("coordinates", "near the lake", "28.5 N 81.3 W"), "not checked")
  expect_equal(cmp("collection_date", "spring 2020", "2020-04-01"), "not checked")
  ...
  expect_equal(cmp("country", "Atlantis", "US"), "not checked")
  ...
  expect_equal(cmp("locality", "Lake Underhill", "Lake Eola"), "not checked")
  expect_equal(cmp("collector", "R. Robins", "Robins, Robert"), "not checked")
  expect_equal(cmp("sex", "Female", "female"), "agree")
  expect_equal(cmp("dev_stage", "adult", "juvenile"), "not checked")
})
...
test_that(".spec_status reports single, the worst pair, or NA", {
  ...
  expect_equal(.spec_status("collector", c(NA, "A. B", "Ann B")), "not checked")
})
