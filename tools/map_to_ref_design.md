...
| `assemble.nf:19` output | APPEND an 11th element, `path("${id}/assemble/${opts_id}/maptoref", optional: true)`, so the loop record of 4.11 is actually published; only declared outputs reach `publishDir`. Optional because the other two assemblers never create that directory. Existing indices are untouched (`opts.assembler` still flows as `[7]`) |
