process assemble {
    ...
    if [ "!{opts.assembler}" = "GetOrganelle" ]; then
        ...
        Rscript -e "MitoPilot::map_to_ref('!{id}', '!{ref}', '!{reads[0]}', '!{reads[1]}', '!{opts.maptoref}', '!{opts.maptoref_consensus}', !{opts.maptoref_iter}, '!{opts.maptoref_topology}', !{genetic_code.intValue()}, !{opts.cpus}, '!{outDir}', '!{opts.maptoref_value}', mapper = '!{opts.maptoref_mapper}')"
        echo "!{opts.maptoref_mapper} !{opts.maptoref} | !{opts.maptoref_consensus} | iterate !{opts.maptoref_iter}" > !{outDir}/opts.txt
        ### ARCHIVE READS ###
        tar -czvf !{outDir}/!{id}_reads.tar.gz *.fastq.gz
        ### work dir info for troubleshooting ####
        echo "Nextflow assemble working directory:" > !{outDir}/NF_work_dir_assemble.txt
