process blast_ref_fetch {
    ...
    output:
        tuple val(id), val(blast_accession), path("${outDir}/blast_ref_annotations.csv"), path("${outDir}/blast_ref_sequence.txt"), path("${outDir}/blast_ref_genetic_code.txt")

    shell:
    outDir = "${id}/assemble/${opts_id}"
    '''
    mkdir -p !{outDir}
    Rscript -e "MitoPilot::fetch_blast_ref('!{blast_accession}', '!{outDir}/blast_ref_annotations.csv', '!{outDir}/blast_ref_sequence.txt', '!{outDir}/blast_ref_genetic_code.txt')"
    '''
}
