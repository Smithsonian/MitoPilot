...
export_files <- function(
  ...
  purrr::walk(IDs, ~{
    ...
    if(length(seq)>1){stop("Multiple sequences found. Export not supported for fragmented assemblies.")}
    ...
    if(file.exists(tbl_fn)){file.remove(tbl_fn)}
